//Strainers

ServerEvents.recipes(event => {

    //Remove
    event.remove({id: /strainers:strainer\/purified_gravel/})
    event.remove({id: "strainers:strainer/purified_sand/prismarine_crystals"})
    event.remove({id: "strainers:strainer/purified_sand/prismarine_shard"})
    event.remove({id: "strainers:strainer/purified_soul_sand/debris_ore_piece"})
    
    //Charged Sand -> Certus
    strainerAuto("submerged:charged_sand", "minecraft:water", "ae2:certus_quartz_dust", 0.35, 0.05, 6, 10);

    //Purifed Soul Sand -> Gold Ore Piece
    strainerAuto("strainers:purified_soul_sand", "minecraft:water", "strainers:gold_ore_piece", 0.2, 0.05, 6, 10);

    //Living Gravel -> Redstone
    strainerAuto("submerged:living_gravel", "minecraft:water", "strainers:redstone_ore_piece", 0.2, 0.05, 5, 10);

    //Pressurized Sand -> Prismarine Crystals, Prismarine Shard, Salt
    strainerAuto("submerged:pressurized_sand", "minecraft:water", "minecraft:prismarine_crystals", 0.1, 0.05, 5, 10);
    strainerAuto("submerged:pressurized_sand", "minecraft:water", "minecraft:prismarine_shard", 0.15, 0.05, 5, 10);
    strainerAuto("submerged:pressurized_sand", "minecraft:water", "alltheores:salt", 0.05, 0.05, 5, 10);

    //Purified Gravel -> Coal Ore Piece
    strainerAuto("strainers:purified_gravel", "minecraft:water", "strainers:coal_ore_piece", 0.2, 0.05, 1, 10);

    //Refined Gravel -> Tin, Copper, Aluminum
    strainerAuto("submerged:refined_gravel", "minecraft:water", "strainers:tin_ore_piece", 0.2, 0.05, 1, 10);
    strainerAuto("submerged:refined_gravel", "minecraft:water", "strainers:copper_ore_piece", 0.2, 0.05, 1, 10);
    strainerAuto("submerged:refined_gravel", "minecraft:water", "strainers:aluminum_ore_piece", 0.2, 0.05, 1, 10);

    //Totemic Infused Gravel -> Iron, Zinc, Lapis
    strainerAuto("submerged:totemic_infused_gravel", "minecraft:water", "strainers:iron_ore_piece", 0.2, 0.05, 4, 10);
    strainerAuto("submerged:totemic_infused_gravel", "minecraft:water", "strainers:zinc_ore_piece", 0.2, 0.05, 4, 10);
    strainerAuto("submerged:totemic_infused_gravel", "minecraft:water", "strainers:lapis_ore_piece", 0.2, 0.05, 4, 10);

    //Auto Strainer Recipes
    colors.forEach(color => {
        strainerAuto(`colors:${color}_leaves`, "minecraft:water", `colors:${color}_apple`, 0.2, 0.05, 1, 10);
    })

    //Auto Strainer Tier Recipes
    function strainerAuto(input, fluid, item, baseChance, addChance, minTier, maxTier) {
        let results = [];
        for (let tier = minTier; tier <= maxTier; tier++) {
            let chance = baseChance + (tier - minTier) * addChance;
            results.push([item, chance, `#strainers:tier_${tier}_meshes`]);
        }
        return event.recipes.strainers.strainer(input, fluid, results);
    }
})
